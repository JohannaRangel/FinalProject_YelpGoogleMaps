...
def datos_nuevos_yelp():
    # URL de la API con el token
    api_url = 'https://api.apify.com/v2/datasets/kGaVaPMnzYF5o8CEF/items?token=apify_api_ZE7FMykxbpsHef9FAMxUqF9esYgIGK2LrElm&unwind=reviews'
    # Realiza la solicitud GET
    response = requests.get(api_url)
...
def run():
    ...
    print('Etl completo reviews Google')
    validación(df,'google_reviews_ulta_beauty',logs)
    print('validación completa google reviews')
    ...
    if filassincargar>0:
        print('filas cargadas correctamente')
        logs=cargar_logs(logs,'verificación de carga completa reviews yelp 2021-2023','yelp reviews')
    else:
        print(f'filas pendientes de cargar{filassincargar}')
